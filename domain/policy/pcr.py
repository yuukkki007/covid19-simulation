...
def distribute_pcr_kit(human_dict: Dict[str, Human], cluster_dict: Dict[str, Cluster]) -> SimulationResult:
    ...
    for i in tqdm(range(SIMULATION_ENV.simulation_days)):
        onset_ids = []
        economic_loss = 0
        freeze_rate = 0.0
        freeze_count = 0
        ...
        for key in human_dict.keys():
            ...
            if human_dict[key].pcr is True:
                human_dict[key].passed_day_after_pcr += 1
                # PCR経過後10日経過していた場合初期化
                if human_dict[key].passed_day_after_pcr == 10:
                    human_dict[key].pcr = False
                    human_dict[key].passed_day_after_pcr = 0
        ...
        # 発症者がロックダウン水準を超える場合かかったことのない人は全員PCR検査を行う
        counts = 0
        for human_key in human_dict.keys():
        ...
        if threshold < counts:
            logger.info("pcr!!!")
            for human_key in human_dict.keys():
                if human_dict[human_key].has_antibodies is False and (human_dict[human_key].pcr is False):
                    # pcr検査キットの値段を経済損失に加える
                    economic_loss += SIMULATION_ENV.PCR_kit_price
                    # 今までかかったことがなく感染していて、まだ発症してない人のうち40%を陽性として判断し凍結する
                    if human_dict[human_key].infected is False:
                        pcr = bool(np.random.choice(a=[True, False], p=[0.4, 0.6]))
                        human_dict[human_key].pcr = True
                        if pcr is True:
                            human_dict[human_key].know_infection = True
                            human_dict[human_key].freeze = pcr
